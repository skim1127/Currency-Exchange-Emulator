class Currency:
    ...
    def __repr__(self):
        ...
        return f"{round(self.value, 2)}" + " " + f"{self.unit}"
    ...
    def __radd__(self, other):
        # This method is similar to __add__(self,other), but occurs when an int or float 
        # tries to add a Currency object. (Treat the int/float as having a USD value.)
        res = self + other
        if self.unit != "USD":
            res.changeTo("USD")
        return res
